...
| 2026-05-26T12:46-04:00 | Does a signed-out user see any posting action before "Login with X"? | Hid the saved "Last proof" panel unless the user is signed in. | `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm verify:goals`, `pnpm build`, `spectacula validate`, and `git diff --check` passed. | The first unauthenticated screen stays focused on X login, not stale proof actions. |
...
| 2026-05-26T13:04-04:00 | Can a tampered saved proof change the "Post to X" destination? | Tightened saved proof validation to require a same-origin proof URL and matching `x.com/intent/tweet` text/proof URL. | `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm verify:goals`, `pnpm build`, `spectacula validate`, and `git diff --check` passed. | Stale local proof actions now need to match the same proof and post text the app created. |
